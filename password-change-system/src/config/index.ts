...
let calibratedCostFactor: number | null = null;

/**
 * Calibrate bcrypt cost factor to achieve ~250ms hash time
 * Run once at startup and cache result
 */
export async function calibrateBcryptCost(): Promise<number> {
  ...
  let cost = config.bcrypt.costFactor;
  const maxCost = 16;
...
export function getBcryptCostFactor(): number {
  return calibratedCostFactor || config.bcrypt.costFactor;
}
...
export async function generateBcryptBenchmark(): Promise<string> {
  const testPassword = 'benchmark_test_password_123';
  const results: Array<{ cost: number; timeMs: number }> = [];
  ...
  results.forEach(({ cost, timeMs }) => {
    let recommendation = '';
    if (cost === 10) recommendation = 'Minimum (fast hardware)';
    else if (cost === 12) recommendation = 'Default (balanced)';
    else if (cost === 14) recommendation = 'Recommended (~250ms target)';
    else if (cost >= 15) recommendation = 'Maximum security';
